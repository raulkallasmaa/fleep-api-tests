...
import { loadConfigAsync, randomBytesAsync, randomUUID } from './utils';
...
let DEFAULT_TIMEOUT_INTERVAL = 5 * 60 * 1000;
...
let FLEEP_USERNAME = process.env.FLEEP_USERNAME || process.env.USER || process.env.LOGNAME || 'noname';
...
function newUserGenerator(display_name) {
    return randomBytesAsync(32)
        .then((rndBuf) => {
            ...
            let srv_user = imap_user;
            let srv_host = imap_server;
            ...
            let m2, m = /^([^<>]*)<([^>]+)>$/.exec(display_name.trim());
            if (m) {
                registerEmail = false;
                display_name = m[1].trim();
                let srv_email = m[2];
                if (imap_test_servers[srv_email]) {
                    srv_email = imap_test_servers[srv_email].email;
                }
                m2 = /(.*)@(.*)/.exec(srv_email);
                if (m2) {
                    srv_user = m2[1].trim();
            ...
            let email = `${srv_user}+${fleep_address}@${srv_host}`;
...
function setup_sys(gpgOnce) {
    ...
    let p2 = loadConfigAsync(cfname2)
        .then((conf) => {
            ...
            // itest_marko_box = json
            let iname = 'itest_' + FLEEP_USERNAME + '_';
            for (let k in conf) {
                if (k.substr(0, iname.length) === iname) {
                    let kind = k.substr(iname.length);
                    imap_test_servers[kind] = JSON.parse(conf[k]);
                }
            }
            return Promise.resolve();
        });
...
class UserCache {
    ...
    setup() {
        return setup_sys()
            .then(() => {
                let sc = new SysClient(BASE_URL, sys_user, sys_password, newUserGenerator, this._filelog);
                this.sysclient = sc;
