...
let DEFAULT_TIMEOUT_INTERVAL = 65000;
...
class UserCache {
    constructor(users, filename, _jasmine) {
        ...
        if (filename) {
            ...
            this.filelog = fileLogger(fnprefix);
        } else {
            this.filelog = console;
        }
                ...
                return sc.quick_register_many(this._users)
                    .then((usermap) => {
                        for (let k in usermap) {
                            this[k] = new TestClient(ENV_HOST, usermap[k], this._magic, smtp_transport, imap_listener, this.filelog);
