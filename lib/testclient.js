...
import { requestAsync, cookieJar, newAgent, waitAsync, randomInt, readFileAsync } from './utils';
...
class TestClient {
    ...
    raw_api_call(path, payload, qs) {
        ...
        return requestAsync(req)
            .then((res) => {
                ...
                this.log.info("[%s] API CALL: %s\nrequest headers: %s\nrequest: %s\nresponse: %s %s\nresponse headers: %s\n", now_str(), get_href(res),
            ...
            .catch((err) => {
                ...
                let status = res.statusCode + ' ' + res.statusMessage;
                this.log.info("[%s] API CALL ERROR: %s\nrequest headers %s\nrequest: %s\nerror: %s %s\n", now_str(), get_href(res),
                              fmt_http_headers(res.request.headers),
                              JSON.stringify(jsbody, null, 2),
                              status,
                              JSON.stringify(res.body, null, 2));
    ...
    raw_api_put(path, filename, qs) {
        ...
        return readFileAsync(filename)
            .then((data) => requestAsync(Object.assign(req, {body: data})))
            .then((res) => {
                // remember full req
                this.last_response = res;
                ...
                this.log.info("[%s] API PUT: %s\nrequest headers: %s\nrequest: %s\nresponse: %s %s\nresponse headers: %s\n", now_str(), get_href(res),
                              fmt_http_headers(res.request.headers),
                              JSON.stringify({bodylen: req.body.length}, null, 2),
                              status,
                              JSON.stringify(res.body, null, 2),
                              fmt_http_headers(res.headers));
            ...
            .catch((err) => {
                let res = err.response;
                if (!res) {
                    this.log.info("[%s] API PUT unknown ERROR: %s%s\nerror: %s\n", now_str(), this.base_url, path, err);
                    return Promise.reject(err);
                }
                let status = res.statusCode + ' ' + res.statusMessage;
                this.log.info("[%s] API PUT ERROR: %s\nrequest headers %s\nrequest: %s\nerror: %s %s\n", now_str(), get_href(res),
                              fmt_http_headers(res.request.headers),
                              JSON.stringify({bodylen: req.body.length}, null, 2),
                              status,
                              JSON.stringify(res.body, null, 2));
    ...
    raw_request(path, req) {
        ...
        return requestAsync(req)
            .then((res) => {
                let status = res.statusCode + ' ' + res.statusMessage;
                this.log.info("[%s] HTTP %s: %s\nrequest headers: %s\nresponse: %s\nresponse headers: %s\n",
                              now_str(), req.method, get_href(res),
                              fmt_http_headers(res.request.headers),
                              status,
                              fmt_http_headers(res.headers));
                ...
                let res = err.response;
                if (!res) {
                    this.log.info("[%s] API PUT unknown ERROR: %s\nerror: %s\n", now_str(), req.uri, err);
                    return Promise.reject(err);
                }
                let status = res.statusCode + ' ' + res.statusMessage;
                this.log.info("[%s] HTTP %s ERROR: %s\nrequest headers %s\nerror: %s %s\n",
                              now_str(), req.method, get_href(res),
                              fmt_http_headers(res.request.headers),
                              status,
                              JSON.stringify(res.body, null, 2));
    ...
    api_put(path, filename) {
        let p = Promise.resolve();
        if (!this.ticket) {
            p = p.then(() => this.login());
        }
        return p.then(() => this.raw_api_put(path, filename, {ticket: this.ticket}))
