        ...
        return this.client.listMessages('INBOX', seq, shortQuery, {byUid: true})
            .then((list) => {
                ...
                for (let i = 0; i < list.length; i++) {
                    ...
                    // detect old messages
                    if (list[i].internaldate) {
                        // internaldate: 22-Jun-2017 16:58:13 +0300
                        let day = parseInt(list[i].internaldate.split('-')[0], 10);
                        if (day !== now.getUTCDate() && del_uids.length < 300) {
                            del_uids.push(list[i].uid);
                        }
                    }
                ...
                // delete old messages
                if (del_uids.length > 0) {
                    p = p.then(() => this.client.deleteMessages('INBOX', del_uids.join(','), {byUid: true}));
                }

                if (uids.length === 0) {
                    return p;
                }
                return p.then(() => this.client.listMessages('INBOX', uids.join(','), fullQuery, {byUid: true}))
                    .then((flist) => {
