...
test(`Test my taskboard info propagation`, function () {
    let state = {};
    return thenSequence([
        // do initial poll
        () => UC.alice.initial_poll(),
        ...
        () => setupConv(state, 'Create task', UC.alice, [UC.bob]),
        // bob assignes task to alice
        () => addEvent(state, UC.bob, et.MESSAGE_ADD__TODO, {
            conversation_id: state.conversation_id,
            message: "Message assigned to alice",
            assignee_ids: [UC.alice.account_id],
        }),
        ...
        // alice waits for the message to arrive with poll
        () => UC.alice.poll_filter({mk_rec_type: 'message', message_nr: state.r_message1.message_nr}),
        () => {
            state.r_message_my_task = UC.alice.matchStream({
                mk_rec_type: 'message',
            ...
            console.log(state.r_message_my_task);
            expect(state.r_message_my_task).toMatchObject({
                mk_message_state: ms.TODO,
                message: '<msg><p>Message assigned to alice</p></msg>',
            });
