...
class Message(object):
    ...
    SIMPLE_FIELDS = (
        ...
        'html_email_data',
        'lock_account_id', 'inbox_nr', 'subject', 'reactions')
    ...
    def show(self, contacts, teams, files, read_msg_nr = None, is_pinboard = False):
        ...
        elif self.mk_message_type in ('closeV1', 'closeV2'):
            data = json.loads(self.message)
            topic = data['topic']
            message = "*** The Fleep account of " + topic + " was deleted.***"
        ...
        if self.reactions:
            reactions = ' %s' % sorted(self.reactions.keys())
        else:
            reactions = ''

        return "{:>4} {:<20} {}{}{} {}{}".format(message_nr, display_name, is_unread, is_edited, is_pinned, message, reactions)
...
class Conversation(object):
    ...
    def __init__(self, cache, conversation_id):
        ...
        self.labellist = cache.labels   #: account labels
        self.orglist = cache.orgs       #: account orgs
        ...
        self.label_ids = []             #: labels user has given to this conversation
    ...
    def update_message(self, message):
        ...
        if m.mk_message_type and m.mk_message_type not in (
                ...
                'separator','show','unshow','no_url_previews','url_previews',
                'bounce','replace', 'call','closeV1','closeV2','delete'):
    ...
    def _update_header(self, c):
        ...
        self.label_ids = c.get('label_ids', self.label_ids)
    ...
    def store(self,
        ...
        return self._sync(
            self.api.conversation_store( self.conversation_id,
    ...
    def show_labels(self):
        labels = ' | '.join([self.labellist[label_id]['label'] for label_id in self.label_ids])
        return labels
    ...
    def show(self, show_flow = True, show_members = True, show_pinboard = True, from_first = False, show_active = False):
        ...
        labels = '\n Labels: ' + self.show_labels() if self.label_ids else ''
        ...
        return "%s%s%s%s%s" % (header, labels, members, pinboard, flow)
...
class ContactList(object):
    ...
    def get_name(self, account_id):
        ...
        n = a.get('display_name') or a['email']
        if n:
            n = n.strip()
        return n
    ...
    def show(self):
        ...
        for contact in contacts_show:
            ...
            if display_name == contact.get('email'):
                display_name =  ''
...
class ReminderList(object):
    ...
    def __init__(self, api):
        self.api = api
        self.reminders = {}                         #: teams by team_id

    def set_reminder(self, a):
        """Insert or update contact
        """
        self.reminders[a['reminder_id']] = a

    def show(self):
        """Display contents of team cache
        """
        return '\n'.join(
           ["%s" % (r_reminder['mk_reminder_type'],) for r_reminder in self.reminders])
...
class FleepCache(object):
    ...
    def __init__(self, url, email = None, password = None, token = None, ticket = None):
        ...
        self.reminders = ReminderList(self.api)
        self.orgs = {}
        self.labels = {}
    ...
    def team_remove(self, team_id):
        retval = self.api.team_remove(team_id)
        self._process_stream(retval['stream'])

    def team_add_members(self, team_id, emails):
        retval = self.api.team_configure(team_id, add_emails = emails)
        self._process_stream(retval['stream'])

    def team_remove_members(self, team_id, emails):
        retval = self.api.team_configure(team_id, remove_emails = emails)
        self._process_stream(retval['stream'])
    ...
    def _process_stream(self, stream):
        ...
        for rec in stream:
            ...
            elif rec['mk_rec_type'] in ['preview','replace']:
                pass
            elif rec['mk_rec_type'] == 'label':
                self.labels[rec['label_id']] = rec
            elif rec['mk_rec_type'] == 'reminder':
                self.reminders.set_reminder(rec)
            elif rec['mk_rec_type'] == 'org_header':
                self.orgs[rec['organisation_id']] = rec
            else:
                logging.warning('Unhandled record type %s', rec['mk_rec_type'])
